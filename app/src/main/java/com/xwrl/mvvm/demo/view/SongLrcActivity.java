...
public class SongLrcActivity extends BaseActivity<SongLrcViewModel> {
    ...
    private void initView(){
        ...
        mSongLrcBinding.songLrcBottomList.setOnClickListener(v -> openActivity(5, null));
    ...
    private void initAnimator(){
        ...
        //设置唱片机械臂的转动中心点（0，0）为左上角。
        //现根据Window画图显示，实际转动点为（44px，44px）px像素，图片宽高为（276x414）
        //籍此 宽高比例 即可 设置为 实际中心转动点。 2023/05/11
        ...
        //在视图加载完成后，通过handler回调去设置 中心转动点。
        mSongLrcBinding.songLrcCenterIvNeedle.post(() -> {
            ConstraintLayout.LayoutParams needleParams = (ConstraintLayout.LayoutParams)
                                        mSongLrcBinding.songLrcCenterIvNeedle.getLayoutParams();
            //设置唱片机械臂的旋转点 x与y
            int pivotX = needleParams.width * 44 / 276;
            mSongLrcBinding.songLrcCenterIvNeedle.setPivotX(pivotX);

            int pivotY = needleParams.height * 44 / 414;
            mSongLrcBinding.songLrcCenterIvNeedle.setPivotY(pivotY);
            //黑胶唱片与专辑图片的大小比例与设置转动中心点同理，去画图查看像素比即可，大概是75%
        });
    }
    ...
    private class MyMediaControllerCallback extends MediaControllerCompat.Callback{
        ...
        public void onSessionEvent(String event, Bundle extras) {
            ...
            }else if (MediaPlayerManager.DYQL_CUSTOM_ACTION_IS_LOVED.equals(event)){
                mSongLrcViewModel.setLoved(extras.getBoolean(
                        MediaPlayerManager.DYQL_CUSTOM_ACTION_IS_LOVED,false));
            }
        }
    }
    ...
    private void sureRecordSize(){
        ...
        params1 = mSongLrcBinding.songLrcCslCenterIvRecord.getLayoutParams();
        size -= dpToPx(2);
        params1.width = size;
        params1.height = size;
        mSongLrcViewModel.setAlbumViewSize(size);
        Log.d(TAG, "sureRecordSize: 黑胶唱片大小为 "+size);
        ...
        params1.height = size;
        mSongLrcViewModel.setAlbumViewSize(size);
        Log.d(TAG, "sureRecordSize: 专辑图片大小为 "+size);
    }
